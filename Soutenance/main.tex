...
\begin{xframe}{Setting}
  ...
  \begin{xitemize}
      ...
      \item $m$-sized \iid learning sample $\S\in\mathcal{Z}^m$, $\S:=\{\z_i\}_{i=1}^{m} \sim \D^{\otimes m} $
      \item Population risk $\Risk_{\D}(h)=\EE_{\z\sim\D}\loss(h, \z)$ and empirical risk $\Riskhat_{\S}(h)=\frac{1}{m}\sum_{i=1}^{m}\loss(h, \z_i)$
      \item Expected risks $\Risk_{\D}(\Q) = \displaystyle\EE_{h\sim\Q}[\Risk_{\D}(h)],$ \hspace{0.1cm} $\Riskhat_{\S}(\Q) = \displaystyle\EE_{h\sim\Q}[\Riskhat_{\S}(h)]$
