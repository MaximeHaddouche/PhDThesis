%!TEX root = main.tex
\chapter[Wasserstein PAC-Bayes in Practice: Generalisation-Driven Learning Algorithms for Deterministic Predictors]{Wasserstein PAC-Bayes in Practice: Genrealisation-Driven Learning Algorithms for Deterministic Predictors}
...
This last condition covers, in particular, the case where $\H$ is an Euclidean space and for any $i$, the distribution $\P_{i,\S}$ is a Dirac mass. This is weaker than the condition \textit{(ii)} of the stochastic kernels sequence in \Cref{chap:online-pb}, but enough to exploit the conditional Fubini lemma (\Cref{l: cond_fubini-chap3}).
...
Then, we exploit the supermartingales used in \Cref{chap: pb-ht} and \cite{chugg2023unified} alongside Ville's inequality (instead of Markov's one) to obtain a high probability bound holding for heavy-tailed losses.
...
An important point is that the variance terms are considered with respect to the prior distributions $\P_{i,\S}$  and not $\Q$ as in \Cref{chap: pb-ht}. This is crucial as these chapters rely on the implicit assumption of order 2 moments, holding uniformly for all $\Q\in\Mcal(\H)$, while we only require this assumption for the prior distributions $(\P_{i,\S})_{i=1,\dots,K}$.
...
To satisfy the assumption of \Cref{theorem:supervised-nnl}, it is enough, by Cauchy Schwarz, to satisfy  $\EE_{h\sim \P_{i,\S},z\sim\D}[\loss(h,z)^2] \le \EE[h^2] + 2V\EE[|h|] +V^2 \leq 1$ for all $\P_{i,\S}$.
On the contrary, \Cref{chap: pb-ht} would require this condition to hold for all $\Q$, which is more restrictive.
...
Online PAC-Bayes bounds are meant to derive online counterparts of classical PAC-Bayesian algorithms as in \Cref{chap:online-pb}, where the KL-divergence acts as a regulariser.
...
\Cref{chap:online-pb} derived from their main theorem, an online counterpart of \Cref{eq:catoni-alg}, proving it comes with guarantees.
...
Instead, for the population risk $\Risk_{\D}(h)$ and the empirical risk $\Riskhat_{\Sm}(h)$ (in \Cref{theorem:supervised-nnl} and \Cref{eq:batch-alg}), we consider the loss defined as $\loss(h, (\x, y)) = \frac{1}{|\Y|}\sum_{y'\ne y} \max(0, 1{-} \eta(h[y]{-}h[y']))$, which is $\eta$-Lipschitz \wrt $h[1],\dots, h[|\Y|]$.
...
\Cref{eq:batch-alg-exp} then optimises a tradeoff between the empirical risk and the regularisation term $\varepsilon\sum_{i=1}^{K} \frac{|\Sm^{i}|}{m} d(h_{\wbf}, h_{\wbf_i})$.
...
We derived new learning algorithms based on Wasserstein PAC-Bayes bounds. Such remarkable empirical results shows the strengths of the optimisation perspective on PAC-Bayes learning. Indeed, we exploited here various paradigms gathered in \Cref{fig: recap-optim}: weak statistical assumptions with stronger geometric ones (heavy-tailed lipschitz losses), the use of deterministic predictors, allowed through the introduction of Wasserstein distance. For the online method, we also involve the vision of prior as initialisation point, which is attenuated theoretically as the learning process goes. Finally, note that for our batch methods to work, we involved data-dependent priors which is an ersatz from the information-theoretic view of PAC-Bayes (\Cref{fig: recap-info}) circumventing this limitation, possibly by exploiting the results of \Cref{chap:gen-flat-minima} in the batch learning case is a promising future lead to reach not only strong performances, but also non-vacuous theoretical guarantees, which is not the case here.  
