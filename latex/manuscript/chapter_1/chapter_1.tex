...
Those assumptions are suited to the learning problem $(\Hcal_2,\Z,\ell)$ of \Cref{ex: neural_net} 
...
% Then, go with the practical part, show how to derive algorithms for Mcall and catoni (and even more) and then, redo a similar paragraph using what is below
%and \citet{dziugaite2017computing} showed that a PAC-Bayesian training allowed to obtain non-vacuous generalisaiton bounds, meaning that using a KL divergence as complexity alongside an information-theoretic training helps to understand the generalisation ability of neural networks. However the assumptions of \Cref{prop: mcall-catoni} raise several points of inquiry:
...
% Also in the practical part, mention the various practical settings where variations of MCAllester or Catoni are used to obtain algorithms with non-vacuous guarantees.
