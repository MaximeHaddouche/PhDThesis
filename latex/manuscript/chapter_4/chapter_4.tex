...
Assume that $\P$ is a probability measure on $\mathbb{R}^d$ such that $d\P(h) \propto \exp(-V(x))$ with $V$ a smooth function such that $Hess(V)\succeq \frac{2}{c_{LS}(\P)}\mathrm{Id}$ (We say that $A\succeq B$ if $A-B$ is a positive semidefinite matrix.).
...
\section{Reaching a flat minimum allows Poincaré posteriors generalising well}
