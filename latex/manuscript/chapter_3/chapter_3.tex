...
Finally, we note that if we assume being able to bound simultaneaously all condtional means and variance (which is strictly less restrictive than bounding the loss),then  \Cref{th: opb-ht} suggests a new online learning objective which is an online counterpart to \Cref{eq: optim_obj}.
...
\end{itemize}
 ...
 \Cref{chap:online-pb} builds a bridge between online learning and generalisation. As seen in \Cref{sec: experiments}, considering online PAC-Bayes procedures mitigates the impact of the prior in the learning process and thus, fit the optimisation view of the prior as in initialisation point (\Cref{fig: recap-optim}), yielding performances comparable to online gradient descent. 
